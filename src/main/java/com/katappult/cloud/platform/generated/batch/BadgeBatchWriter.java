...
@Component("BadgeBatchWriter")
@Scope("step")
public class BadgeBatchWriter implements ItemWriter<AbstractLineBean> {

    private static final Logger LOGGER = LoggerFactory.getLogger(BadgeBatchWriter.class);
    ...
    public void write(final List<? extends AbstractLineBean> items) throws Exception {
        ...
        while(entityIterator.hasNext()) {
          ...
          if(command.startsWith("ADD_")){
              ...
              Badge entity = (Badge) line.getRootEntity();
          ...
          if(command.startsWith("UPDATE_")){
              Badge transientEntity = (Badge) line.getRootEntity();
