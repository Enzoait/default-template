...
import com.katappult.cloud.platform.generated.model.Badge;
...
@ManagedBean
@ViewScoped
public class BadgeController extends BaseCRUDController<Badge> implements Serializable {

  private static final Logger LOGGER = LoggerFactory.getLogger(BadgeController.class);

  private String entityName = "Badge";
  private String entityNameLower = "badge";
  ...
  private Badge newElement = new Badge();
  private Badge detailsElement = null;
  ...
  @Inject
  private IBadgeService entityService;
  ...
  public void _doDeleteElement(Long oid) {
    ...
    IPersistable elementToDelete = services.persistableService().findById(oid, Badge.class);
    ...
    try {
      entityService.delete((Badge) elementToDelete, getWorkingContainer());
    } catch (Exception ex) {
  ...
  public String _doCreateNewElementAction() {
    ...
    try {
      ...
      Badge createdElement = entityService.create(uiAttributes, getWorkingContainer());

      reloadChartDatas();

      newElement = new Badge();
      if(!isAdminRequest()) {
  ...
  public void _doUpdateElement() {
    ...
    try {
      ...
      Badge element = entityService.update(uiAttributes, getWorkingContainer());
  ...
  private void loadDetails(boolean refresh) {
    ...
    if (shouldReLoadDatas || refresh) {
      ...
      if (shouldReLoadDatas) {
        Badge element =
            (Badge) services.persistableService().findById(targetId, Badge.class);
        elementMetadata.setElement(element);
        detailsElement = element;
      }
      ...
      if (refresh && previousId != null) {
        Badge element =
            (Badge)
                services
                    .persistableService()
                    .findByIdFetchType(previousId, Badge.class);
        elementMetadata.setElement(element);
        detailsElement = element;
      }
  ...
  @Override
  public Class<Badge> getTargetEntityClass() {
    return Badge.class;
  }
  ...
  public Badge getNewElement() {
    return newElement;
  }

  public void setNewElement(Badge newElement) {
  ...
  public Badge getDetailsElement() {
    return detailsElement;
  }

  public void setDetailsElement(Badge detailsElement) {
