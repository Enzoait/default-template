...
import com.katappult.cloud.platform.generated.model.Pointer;
...
@ManagedBean
@ViewScoped
public class PointerController extends BaseCRUDController<Pointer> implements Serializable {

  private static final Logger LOGGER = LoggerFactory.getLogger(PointerController.class);

  private String entityName = "Pointer";
  private String entityNameLower = "pointer";
  ...
  private Pointer newElement = new Pointer();
  private Pointer detailsElement = null;
  ...
  @Inject
  private IPointerService entityService;
  ...
  public void _doDeleteElement(Long oid) {
    ...
    IPersistable elementToDelete = services.persistableService().findById(oid, Pointer.class);
    ...
    try {
      entityService.delete((Pointer) elementToDelete, getWorkingContainer());
    } catch (Exception ex) {
  ...
  public String _doCreateNewElementAction() {
    ...
    try {
      ...
      Pointer createdElement = entityService.create(uiAttributes, getWorkingContainer());

      reloadChartDatas();

      newElement = new Pointer();
      if(!isAdminRequest()) {
  ...
  public void _doUpdateElement() {
    ...
    try {
      ...
      Pointer element = entityService.update(uiAttributes, getWorkingContainer());
  ...
  private void loadDetails(boolean refresh) {
    ...
    if (shouldReLoadDatas || refresh) {
      ...
      if (shouldReLoadDatas) {
        Pointer element =
            (Pointer) services.persistableService().findById(targetId, Pointer.class);
        elementMetadata.setElement(element);
        detailsElement = element;
      }
      ...
      if (refresh && previousId != null) {
        Pointer element =
            (Pointer)
                services
                    .persistableService()
                    .findByIdFetchType(previousId, Pointer.class);
        elementMetadata.setElement(element);
        detailsElement = element;
      }
  ...
  @Override
  public Class<Pointer> getTargetEntityClass() {
  ...
  public Pointer getNewElement() {
    return newElement;
  }

  public void setNewElement(Pointer newElement) {
  ...
  public Pointer getDetailsElement() {
    return detailsElement;
  }

  public void setDetailsElement(Pointer detailsElement) {
