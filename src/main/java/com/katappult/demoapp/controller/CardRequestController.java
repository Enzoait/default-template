...
import com.katappult.cloud.platform.generated.model.CardRequest;
...
@ManagedBean
@ViewScoped
public class CardRequestController extends BaseCRUDController<CardRequest> implements Serializable {

  private static final Logger LOGGER = LoggerFactory.getLogger(CardRequestController.class);

  private String entityName = "CardRequest";
  private String entityNameLower = "cardRequest";
  ...
  private CardRequest newElement = new CardRequest();
  private CardRequest detailsElement = null;
  ...
  @Inject
  private ICardRequestService entityService;
  ...
  public void _doDeleteElement(Long oid) {
    ...
    IPersistable elementToDelete = services.persistableService().findById(oid, CardRequest.class);
    ...
    try {
      entityService.delete((CardRequest) elementToDelete, getWorkingContainer());
    } catch (Exception ex) {
  ...
  public String _doCreateNewElementAction() {
    ...
    try {
      ...
      CardRequest createdElement = entityService.create(uiAttributes, getWorkingContainer());

      reloadChartDatas();

      newElement = new CardRequest();
      if(!isAdminRequest()) {
  ...
  public void _doUpdateElement() {
    ...
    try {
      ...
      CardRequest element = entityService.update(uiAttributes, getWorkingContainer());
  ...
  private void loadDetails(boolean refresh) {
    ...
    if (shouldReLoadDatas || refresh) {
      ...
      if (shouldReLoadDatas) {
        CardRequest element =
            (CardRequest) services.persistableService().findById(targetId, CardRequest.class);
        elementMetadata.setElement(element);
        detailsElement = element;
      }
      ...
      if (refresh && previousId != null) {
        CardRequest element =
            (CardRequest)
                services
                    .persistableService()
                    .findByIdFetchType(previousId, CardRequest.class);
        elementMetadata.setElement(element);
        detailsElement = element;
      }
  ...
  @Override
  public Class<CardRequest> getTargetEntityClass() {
  ...
  public CardRequest getNewElement() {
    return newElement;
  }

  public void setNewElement(CardRequest newElement) {
  ...
  public CardRequest getDetailsElement() {
    return detailsElement;
  }

  public void setDetailsElement(CardRequest detailsElement) {
