...
import PointerAdd from "_generated/Pointer/PointerAdd";
import {PointerService} from "_services/_generated/Pointer.services";
import PointerDetails from "_generated/Pointer/PointerDetails";
...
function PointerList(props) {
    ...
    const exportConfig = await DataExportService.getByInternalName('EXPORT_POINTER', containerId).then(response => {
        return response
    })
    ...
    if(!commons.isRequestError(exportConfig)){
        ...
        DataExportService.runExport(id, form, containerId).then(response => {
            ...
            if(response){
                ...
                FileSaver.saveAs(blob, "Pointer.csv");
            }
        }).catch(error => {
            toast.error("Echec de l'export des données")
        })
...
// METHOD
    ...
    const wizardContent = (wizardCloseFunction) => {
        return <PointerAdd onCreateSuccess={() => onCreateSuccess(wizardCloseFunction)} {...props}/>
    }
    ...
    const loadDatas = (index) => {
        ...
        PointerService.listEntity(index, pageSize, props.containerId, params).then(response => {
    ...
    const searchTermUpdated = (e) => {
        ...
        if(searchTerm){
            PointerService.searchEntity(0, 10, searchTerm , props.containerId).then(response => {
                setPage(0)
                setDatas(response.data)
    ...
    const deleteRow = (id) => {
        PointerService.deleteEntity(id, props.containerId).then(response => {
...
{name: "Identifier", dataField: "attributes.identifier", headerClass: "td-left", className: "td-left"},
...
{/*ACTIONS*/}
        ...
        return <PointerDetails {...props} refresh={refresh} forwardToDetailsUrl={null}/>
    }
}
