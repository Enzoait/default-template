...
import ResourceAccessAdd from "_generated/ResourceAccess/ResourceAccessAdd";
import {ResourceAccessService} from "_services/_generated/ResourceAccess.services";
import ResourceAccessDetails from "_generated/ResourceAccess/ResourceAccessDetails";
...
function ResourceAccessList(props) {
    ...
    const exportConfig = await DataExportService.getByInternalName('EXPORT_RESOURCEACCESS', containerId).then(response => {
        return response
    })
    ...
    if(!commons.isRequestError(exportConfig)){
        ...
        DataExportService.runExport(id, form, containerId).then(response => {
            ...
            if(response){
                ...
                FileSaver.saveAs(blob, "ResourceAccess.csv");
            }
        }).catch(error => {
            toast.error("Echec de l'export des données")
        })
...
// METHOD
    ...
    const wizardContent = (wizardCloseFunction) => {
        return <ResourceAccessAdd onCreateSuccess={() => onCreateSuccess(wizardCloseFunction)} {...props}/>
    }
    ...
    const loadDatas = (index) => {
        ...
        ResourceAccessService.listEntity(index, pageSize, props.containerId, params).then(response => {
    ...
    const searchTermUpdated = (e) => {
        ...
        if(searchTerm){
            ResourceAccessService.searchEntity(0, 10, searchTerm , props.containerId).then(response => {
                setPage(0)
                setDatas(response.data)
    ...
    const deleteRow = (id) => {
        ResourceAccessService.deleteEntity(id, props.containerId).then(response => {
    ...
    const itemsDatatableRowItemName = (v, i) => {
        return <td className={'td-left table-info-link'}>
            <a href='#/details' onClick={(e) => displayDetails(e, v.id)}>
                {v.name && v.name.length > 0 ? v.name : "> Infos"}
            </a>
        </td>
    }
    ...
    const table = () => {
        ...
        let tableConfig ={
            columnsConfig: [
                {name:'Name', displayComponent: (v, i) => itemsDatatableRowItemName(v,i), dataField: 'attributes', headerClass: 'td-left'},
{name: "Location", dataField: "attributes.location", headerClass: "td-left", className: "td-left"},
...
{/*ACTIONS*/}
        ...
        return <ResourceAccessDetails {...props} refresh={refresh} forwardToDetailsUrl={null}/>
    }
}
