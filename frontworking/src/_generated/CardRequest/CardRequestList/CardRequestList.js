...
import CardRequestAdd from "_generated/CardRequest/CardRequestAdd";
import {CardRequestService} from "_services/_generated/CardRequest.services";
import CardRequestDetails from "_generated/CardRequest/CardRequestDetails";
...
function CardRequestList(props) {
    ...
    const exportConfig = await DataExportService.getByInternalName('EXPORT_CARDREQUEST', containerId).then(response => {
        return response
    })
    ...
    if(!commons.isRequestError(exportConfig)){
        ...
        DataExportService.runExport(id, form, containerId).then(response => {
            ...
            if(response){
                ...
                FileSaver.saveAs(blob, "CardRequest.csv");
            }
        }).catch(error => {
            toast.error("Echec de l'export des données")
        })
...
// METHOD
    ...
    const wizardContent = (wizardCloseFunction) => {
        return <CardRequestAdd onCreateSuccess={() => onCreateSuccess(wizardCloseFunction)} {...props}/>
    }
    ...
    const loadDatas = (index) => {
        ...
        CardRequestService.listEntity(index, pageSize, props.containerId, params).then(response => {
    ...
    const searchTermUpdated = (e) => {
        ...
        if(searchTerm){
            CardRequestService.searchEntity(0, 10, searchTerm , props.containerId).then(response => {
                setPage(0)
                setDatas(response.data)
    ...
    const deleteRow = (id) => {
        CardRequestService.deleteEntity(id, props.containerId).then(response => {
    ...
    const itemsDatatableRowItemName = (v, i) => {
        return <td className={'td-left table-info-link'}>
            <a href='#/details' onClick={(e) => displayDetails(e, v.id)}>
                {v.description && v.description.length > 0 ? v.description : "> Infos"}
            </a>
        </td>
    }
    ...
    const table = () => {
        ...
        let tableConfig ={
            columnsConfig: [
                {name:'Description', displayComponent: (v, i) => itemsDatatableRowItemName(v,i), dataField: 'attributes', headerClass: 'td-left'},
{name: "Expiration date", dataField: "attributes.requestExpirationDate", type: "datetime", dateFormat: "DD/MM/YYYY HH:mm", headerClass: "td-left", className: "td-left"},
...
{/*ACTIONS*/}
        ...
        return <CardRequestDetails {...props} refresh={refresh} forwardToDetailsUrl={null}/>
    }
}
